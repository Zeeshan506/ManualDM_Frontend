          ...
          <>
            <div className="md:hidden space-y-2.5">
              {leads.map((lead) => (
                <Card
                  key={lead.id}
                  className="relative overflow-hidden border-slate-200 dark:border-slate-800"
                >
                  ...
                  <CardHeader className="pb-1 px-3 pt-3">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <Badge className={`text-white border-none shadow-sm flex items-center gap-1 text-[10px] px-2 py-0.5 shrink-0 ${
                        isLeadHot(lead.lastActive)
                          ? 'bg-gradient-to-r from-blue-600 to-indigo-600'
                          : 'bg-gradient-to-r from-slate-500 to-slate-600'
                      }`}>
                        <Zap className="h-2.5 w-2.5 fill-current" />
                        <span>{isLeadHot(lead.lastActive) ? 'Hot' : 'Cold'}</span>
                      </Badge>
                    </div>
                  </CardHeader>

                  <CardContent className="px-3 pb-2 space-y-1">
                    <div className="flex items-center gap-1.5 text-[10px] rounded-md bg-slate-100/60 dark:bg-slate-900/50 px-2 py-1.5">
                      <User className="h-3 w-3 text-slate-400 shrink-0" />
                      <span className="text-slate-600">{lead.igsid || "N/A"}</span>
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] rounded-md bg-slate-100/60 dark:bg-slate-900/50 px-2 py-1.5">
                  ...
                  <CardFooter className="pt-0 p-3 border-t bg-slate-50/80 dark:bg-slate-900/40">
            ...
            <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {leads.map((lead) => (
                ...
                >
                  ...
                  <CardHeader className="pb-1.5 px-3.5 pt-3.5">
                    <div className="flex justify-between items-start gap-2">
                      <div className="space-y-0.5 overflow-hidden">
                        <CardTitle className="text-[13px] md:text-sm font-bold truncate group-hover:text-primary transition-colors">
                          {lead.name || lead.igsid || "Anonymous Lead"}
                        </CardTitle>
                      ...
                      <Badge className={`text-white border-none shadow-sm flex items-center gap-1 text-[10px] px-2 py-0.5 flex-shrink-0 ${
                        isLeadHot(lead.lastActive)
                          ? 'bg-gradient-to-r from-blue-600 to-indigo-600'
                          : 'bg-gradient-to-r from-slate-500 to-slate-600'
                      }`}>
                  ...
                  <CardContent className="flex-grow space-y-1.5 px-3.5 pb-2.5">
                    <div className="grid grid-cols-1 gap-1">
                      <div className="flex items-center gap-1.5 p-1.5 rounded-md bg-slate-100/50 dark:bg-slate-900/50">
                        <User className="h-3 w-3 text-slate-400 flex-shrink-0" />
                        <div className="flex flex-col">
                          <span className="text-[9px] uppercase font-bold text-slate-400 leading-none mb-0.5">Source ID</span>
                          <span className="text-[10px] font-medium truncate">{lead.igsid || "N/A"}</span>
                        </div>
                      </div>
                  ...
                  <CardFooter className="pt-1.5 border-t bg-slate-50/80 dark:bg-slate-900/40 p-3">
                    <div className="w-full">
                      <ClaimLeadButton leadId={lead.id} />
                    </div>
                  </CardFooter>
